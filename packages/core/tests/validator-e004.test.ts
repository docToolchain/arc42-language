...
describe("E004 — interface between non-building-block", () => {
  test("emitted when interface references a non-building-block element", () => {
  ...
  test("NOT emitted for valid actor↔building-block interface", () => {
    const ws = makeWorkspace([
  ...
  test("emitted for actor↔actor interface (no building-block on either side)", () => {
    const ws = makeWorkspace([
      { kind: "actor", id: "actor-1", title: "User", type: "person", loc: loc(1) },
