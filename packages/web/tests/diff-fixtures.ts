...
import * as story from "../../../scripts/bookstore-evolution.ts";
...
/** A repository with the bookstore example as its only commit, tagged `v1.0` (demos). */
export function createBookstoreRepository(): string {
  const root = join(mkdtempSync(join(tmpdir(), "arc42-demo-")), "bookstore");
  story.createBookstoreRepository(root);
  return root;
}

/** A repository telling the bookstore's evolution story (see scripts/bookstore-evolution.ts). */
