...
Catalog search on PostgreSQL full-text search is starting to show its limits as the catalog grows past 500,000 titles. We're prototyping a dedicated search service backed by Elasticsearch to support fuzzy matching and faceted search.

```arc42
:::decision
id: dec-ecs-fargate
title: Introduce a dedicated Elasticsearch-backed search service
status: proposed
addresses: qg-performance
:::
```
